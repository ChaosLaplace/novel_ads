...
class Collect extends Base
{
    ...
    public function collect_thread($id){
        ...
        return $this->success($return['msg'], json_encode($return), ['title'=>$return['title'],'url'=>$return['reurl'],'status'=>$return['status'],'count'=>$data['count'],'current'=>$data['current']]);
    }
    ...
    public function collect_chapter_thread($id){
    	...
    	if ( $chapter_data ) {
    		...
    		$getchapter = Gather::get_chapter_content($chapter_data['collect_id'], $chapter_data['reurl']);
            if ( $getchapter !== false ) {
